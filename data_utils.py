...
class DataParser(object):
  ...
  #-------------------------DATA PREPROCESSING-------------------------#
    self.short_data, self.long_data = self.data_split(self.train_conjectures)
    self.train_conjectures = self.data_merge(self.short_data, self.long_data)
  #-------------------------DATA PREPROCESSING-------------------------#
  ...
  #-------------------------NEW DATA FUNCTIONS-------------------------#
  def data_split(self, conjectures):
    # input is a set of data entries
    # returns 2 sets of data, split in half by number of steps
    ...
    # get total number of steps for each entry
    ...
    avg_count = (np.sum(pos_step_counts) + np.sum(neg_step_counts))/len(conjectures)

    #assign the data to the new split arrays
    for (key, value) in conjectures.items():
        num_steps = value['+'] + value['-']
        if len(num_steps) < avg_count:
            short_proofs[key] = value
        else:
            long_proofs[key] = value

    return short_proofs, long_proofs

  def data_merge(self, short_data, long_data, w = 0.5):
      #merges the short and long dictionaries, pulling a random amount of entries from each based on w
      new_data = {}
      for i in range(int(len(short_data)*w)):
          rem_key = random.choice(list(short_data.keys()))
          new_data[rem_key] = short_data[rem_key]
          del short_data[rem_key]
      for i in range(int((len(long_data))*(1-w))):
          rem_key = random.choice(list(long_data.keys()))
          new_data[rem_key] = long_data[rem_key]
          del long_data[rem_key]
      return new_data
  #-------------------------NEW DATA FUNCTIONS-------------------------#
  ...
  def draw_random_batch_of_steps(self, dataset, split='train', encoding='integer', max_len=256, batch_size=128):
    if dataset == 1:
      all_conjectures = self.short_data
    else:
      all_conjectures = self.long_data
  ...
  def draw_batch_of_steps_in_order(self, conjecture_index=0, step_index=0, split='train', encoding='integer', max_len=256, batch_size=128):
    if split == 'train':
  ...
  def training_steps_generator(self, dataset, encoding='integer', max_len=256, batch_size=128):
    while 1:
      yield self.draw_random_batch_of_steps(dataset, 'train', encoding, max_len, batch_size)

  def validation_steps_generator(self, encoding='integer', max_len=256, batch_size=128):
    conj_index, step_index = 0, 0
    while 1:
      data, (conj_index, step_index) = self.draw_batch_of_steps_in_order(conj_index, step_index, 'val', encoding, max_len, batch_size)
      yield data

  def draw_batch_of_steps_and_conjectures_in_order(self, conjecture_index=0, step_index=0, split='train', encoding='integer', max_len=256, batch_size=128):
    if split == 'train':
      all_conjectures = self.train_conjectures
      conjecture_names = self.train_conjectures_names
    elif split == 'val':
  ...
  def draw_random_batch_of_steps_and_conjectures(self, split='train', encoding='integer', max_len=256, batch_size=128):
    if split == 'train':
      all_conjectures = self.train_conjectures
    elif split == 'val':
      all_conjectures = self.val_conjectures
  ...
  def training_steps_and_conjectures_generator(self, encoding='integer', max_len=256, batch_size=128):
    while 1:
      yield self.draw_random_batch_of_steps_and_conjectures(
          'train', encoding, max_len, batch_size)

  def validation_steps_and_conjectures_generator(self, encoding='integer', max_len=256, batch_size=128):
    conj_index, step_index = 0, 0
    while 1:
